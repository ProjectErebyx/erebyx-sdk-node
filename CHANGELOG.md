...
- **`JsLoadContextOptions.mode` + `.specializationName` are not part of the v0.1 surface** — matches the substrate's launch surface. `mode` was a single-value enum (`"session"`); specialization-loading is a v0.2 feature. The substrate still accepts these fields on the wire if forwarded, but they're not part of the customer-facing TypeScript surface for v0.1.
- `JsSaveResult.extra` carries the substrate's queue envelope under the `enrichment` key (shape `{queued, queue_id}`).
